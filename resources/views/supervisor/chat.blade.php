      ...
      <div class="bg-white rounded shadow-lg overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b">
          ...
          <div class="flex items-center space-x-4">
            <a href="{{ route('supervisor.students.index') }}" class="text-sm text-gray-700">Back</a>
          </div>
        </div>

        <main class="p-6">
          @if(!$student)
            <div class="p-4 border rounded">Select a student to chat with.</div>
          @else
            <div class="mb-4">
              <div class="font-semibold">Chat with: {{ $student->name }} ({{ $student->subject_course ?? 'N/A' }})</div>
            </div>
            ...
            <form id="sendForm" method="POST" action="{{ route('supervisor.chat.send') }}" class="mt-4 flex">
              @csrf
            ...
            <script>
              ...
              setInterval(async () => {
                try {
                  const res = await fetch('{{ route('supervisor.chat.messages') }}?student_id=' + studentId, {headers: {'X-Requested-With':'XMLHttpRequest'}});
                  if (!res.ok) return;
